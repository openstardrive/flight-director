...
export default class Contact extends React.Component {
  ...
  buildDestinations(contact) {
    ...
    contact.destinations.forEach((d, i) => {
      destinations.push(<line className="destination" key={`${contact.id}-destination-${i}`} x1={start.x * scaleMultiplier} y1={start.y * scaleMultiplier} x2={d.x * scaleMultiplier} y2={d.y * scaleMultiplier} strokeDasharray=".01,.01"/>)
      start.x = d.x
      start.y = d.y
    })
    return destinations
  ...
  render() {
    ...
    return (
      <g>
        ...
        <circle className="contact" key={contact.id} cx={contact.position.x * scaleMultiplier} cy={contact.position.y * scaleMultiplier} r=".03" onMouseDown={e => this.onContactMouseDown(e, contact)}/>
      </g>
    )
  }
}
