...
class Sensors extends React.Component {
  ...
  createNewContact() {
    ...
    let contact = {name, position: {x: -190, y: -190}}
  ...
  onContactSelected(contact, append) {
    console.log('selected contact', contact.name, 'append:', append)
  ...
  moveContact(event) {
    if (this.state.movingContact) {
      let nextDestination = {
        x: event.clientX - this.state.rect.left - 200,
        y: event.clientY - this.state.rect.top - 200
      }
  ...
  onMouseUp(event) {
    if (this.state.movingContact) {
      ...
      if (event.shiftKey) {
        ...
        this.props.dispatch(updateContact(contact, destinations, 10000))
  ...
  renderDragLines() {
    if (this.state.movingContact && this.state.nextDestination) {
      ...
      this.state.destinations.forEach((d, i) => {
        next = d
        lines.push(<line key={`destination-${i}`} x1={current.x} y1={current.y} x2={next.x} y2={next.y} stroke="cyan" strokeWidth="1"/>)
        current = d
      })

      next = this.state.nextDestination;
      lines.push(<line key="destination-n" x1={current.x} y1={current.y} x2={next.x} y2={next.y} stroke="cyan" strokeWidth="1"/>)
  ...
  render() {
    ...
    return (
      <div id="sensors" className="system">
        <h2>Sensors</h2>
        <svg width={400} height={400} viewBox="-200 -200 400 400" style={{backgroundColor: '#666'}} onMouseEnter={e => this.onMouseEnter(e)} onMouseMove={e => this.moveContact(e)} onMouseUp={e => this.onMouseUp(e)}>
          <Grid/>
          <g>
            {this.props.sensors.contacts.map(contact => <Contact key={contact.id} contact={contact} onSelected={(x, y) => this.onContactSelected(x, y)}/>)}
          </g>
