  ...
  renderDragLines() {
    if (this.state.movingContact && this.state.nextDestination) {
      ...
      this.state.destinations.forEach((d, i) => {
        next = d
        lines.push(<line key={`destination-${i}`} x1={current.x * this.scaleMultiplier} y1={current.y * this.scaleMultiplier} x2={next.x * this.scaleMultiplier} y2={next.y * this.scaleMultiplier} stroke="cyan" strokeWidth=".003"/>)
        current = d
      })

      next = this.state.nextDestination;
      lines.push(<line key="destination-n" x1={current.x * this.scaleMultiplier} y1={current.y * this.scaleMultiplier} x2={next.x * this.scaleMultiplier} y2={next.y * this.scaleMultiplier} stroke="cyan" strokeWidth=".003"/>)
  ...
  render() {
    ...
    return (
      <div id="sensors" className="system">
        <h2>Sensors</h2>
        <svg width={400} height={400} viewBox="-1 -1 2 2" style={{backgroundColor: '#666'}} onMouseEnter={e => this.onMouseEnter(e)} onMouseMove={e => this.moveContact(e)} onMouseUp={e => this.onMouseUp(e)}>
          <Grid/>
          <g>
